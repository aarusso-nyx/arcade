import { DELTA, getCurrentTheme, type SnakePalette } from '../../../core';
import type { SnakeConfig } from './config';
import { pixelFontFamily } from './sprites';
import type { SnakeState } from './types';

/**
 * All colours come from the active theme's `games.snake` palette. The engine
 * itself is theme-agnostic; the renderer just looks up the palette on every
 * call, which is cheap and keeps live theme switches instantaneous — no
 * component needs to pass a theme argument, no re-mount is required.
 */
function palette(): SnakePalette {
  return getCurrentTheme().games.snake;
}
...
export function render(
  ...
  drawGrid(ctx, cfg, p);
  if (state.food) drawFood(ctx, state.food.col, state.food.row, cfg, nowMs, p.food, 1);
  if (state.bonus) {
    const remaining = state.bonus.expiresAtMs - nowMs;
    const pulse = remaining > 0 ? 1 : 0;
    drawFood(ctx, state.bonus.cell.col, state.bonus.cell.row, cfg, nowMs, p.bonus, pulse, true);
  }
  const drawAlpha = state.status === 'playing' ? Math.max(0, Math.min(1, alpha)) : 0;
  drawSnake(ctx, state, cfg, drawAlpha, p);
  drawGridBorder(ctx, cfg, p);

  drawHud(ctx, state, hud.highScore, cfg, p);

  if (state.status === 'paused') drawPauseOverlay(ctx, w, h, nowMs, p);
  else if (state.status === 'gameover') {
    const cause = state.deathCause === 'wall' ? 'Hit a wall' : 'Bit yourself';
    drawOverlay(ctx, w, h, 'Game over', `${cause} — Enter to restart`, p);
  } else if (state.status === 'cleared') {
    drawOverlay(ctx, w, h, 'Board cleared!', 'Enter for a new game', p);
  } else if (state.status === 'idle') {
    drawOverlay(ctx, w, h, 'Snake', 'Arrows or WASD — Enter to start', p);
  }
}
...
function drawGrid(ctx: CanvasRenderingContext2D, cfg: SnakeConfig, p: SnakePalette): void {
  const w = cfg.cols * cfg.cellSize;
  const h = cfg.rows * cfg.cellSize;
...
function drawGridBorder(ctx: CanvasRenderingContext2D, cfg: SnakeConfig, p: SnakePalette): void {
  const w = cfg.cols * cfg.cellSize;
  const h = cfg.rows * cfg.cellSize;

  ctx.strokeStyle = p.gridBorder;
  ctx.lineWidth = 2;
...
/**
 * Themed "REPLAY" badge drawn on top of the playfield during replay
 * playback. Kept in the renderer module so all Snake visuals live together;
 * `game.ts` orchestrates when to call it.
 */
